    ...
    for (std::size_t i = 0; i < chainlayoutContent->size(); i++) {
        ...
        for (auto&& link : cont->getLinks(true)) {
            ...
            for (auto&& [key, value] : link) {
                indexList = { i, j, k };
                addValue(values,
                    cs->getItemPath(indexList, { ConfigVal::A_BOXLAYOUT_CHAIN, ConfigVal::A_BOXLAYOUT_CHAIN_LINKS, ConfigVal::A_BOXLAYOUT_CHAIN_LINK }, "key"),
                    cs->option, ConfigVal::A_BOXLAYOUT_CHAIN_LINK, key);
                addValue(values,
                    cs->getItemPath(indexList, { ConfigVal::A_BOXLAYOUT_CHAIN, ConfigVal::A_BOXLAYOUT_CHAIN_LINKS, ConfigVal::A_BOXLAYOUT_CHAIN_LINK }, "value"),
