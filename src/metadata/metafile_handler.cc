...
CueSheetHandler::CueSheetHandler(const std::shared_ptr<Context>& context, std::shared_ptr<Content> content)
    : MetadataHandler(context)
    , content(std::move(content))
{
}

bool CueSheetHandler::fillMetadata(const std::shared_ptr<CdsObject>& obj)
{
    bool result = false;
#ifdef HAVE_JS
    content->parseCueSheet(obj, obj->getLocation());
    result = true;
#endif
    return result;
}

std::unique_ptr<IOHandler> CueSheetHandler::serveContent(const std::shared_ptr<CdsObject>& obj, const std::shared_ptr<CdsResource>& resource)
{
    return {};
}
