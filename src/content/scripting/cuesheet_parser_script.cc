...
duk_ret_t jsAddCueObject(duk_context* ctx)
{
    ...
    try {
        ...
        // self->getDatabase()->updateObject(cdsObj, nullptr);
...
void CuesheetParserScript::processObject(const std::shared_ptr<CdsObject>& obj, const fs::path& path)
{
    log_debug("start");
    ...
    auto parent = item->getParentID() != INVALID_OBJECT_ID ? std::dynamic_pointer_cast<CdsContainer>(database->loadObject(item->getParentID())) : nullptr;
    currentLine = new char[ONE_TEXTLINE_BYTES];
    currentLine[0] = '\0';

    ScriptingRuntime::AutoLock lock(runtime->getMutex());
    try {
        call(obj, parent, cuesheetFunction, path, "");
    ...
    log_debug("Done cuesheet {} ({})...", obj->getLocation().string(), obj->getID());

    cleanUp();
}

std::pair<std::shared_ptr<CdsObject>, int> CuesheetParserScript::createObject2cdsObject(const std::shared_ptr<CdsObject>& origObject, const std::string& rootPath)
{
    return { {}, INVALID_OBJECT_ID };
}

bool CuesheetParserScript::setRefId(const std::shared_ptr<CdsObject>& cdsObj, const std::shared_ptr<CdsObject>& origObject, int pcdId)
{
